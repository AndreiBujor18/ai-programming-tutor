...
- Added `aptutor audit-isolated` with twelve authored hostile-code probes plus a
  ...
  accepted yet.
- Added `aptutor inspect-host`, a bounded read-only preflight with 17 fail-closed
  Linux/rootless-Docker checks, allowlisted non-identifying output, and explicit
  `not_attested` manual requirements. Added an external operations-record template;
  neither artifact accepts a host or authorizes natural-code processing.
...
- Completed focused Windows/Docker Desktop integration acceptance for the worker:
  profile 0.2 passed all twelve hostile-code probes plus cleanup, a 5/5 reference
  and bounded 0/5 starter, source-free output, automatic container removal, and no
  Git-visible private result artifact. Dedicated-host acceptance remains open.
